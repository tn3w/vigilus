...
const canvasHash = await crypto.subtle.digest('SHA-256', 
...
const audioSignature = crypto.subtle.digest('SHA-256',
...
const iceSignature = crypto.subtle.digest('SHA-256',
...
try {
  const worker = new Worker(URL.createObjectURL(
    new Blob(['1+1'], { type: 'application/javascript' })
  ));
  hasWebWorker = true;
  worker.terminate();
} catch (e) {
